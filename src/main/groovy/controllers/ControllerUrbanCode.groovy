...
populateParams.put("NODE", swarmNode.getNodeName());
...
def jobs = ["stepGitUrbanCodeCreateSnapshotSH",
	"stepGitUrbanCodeCompleteSnapshotSH"];
		...
		jobs.each { String job ->
			if(!job.startsWith("#")) {
				...
				if (ok) {
					...
					// Si el job no es bloqueante tendra el parametro "block" a "false".
					//En todo caso el resultado sera "SUCCESS".
					if (jobParams["block"]=="false") {
						...
						// Si el job es bloqueante tendra el parametro "block" distinto de "false".
					} else {
...
// Funcion que nos permite referirnos al nodo levantado en el contenedor Docker
def getNode(String handle) {
	// Se puede sacar a una configuracion externa
	long msecWait = 500;
	int triesLeft = 10;
...
// Ejecucion solo de los jobs que no formen parte de listaSkips

...
class UrbanCodeSnapshotGenerator extends Loggable {
	...
	public void snapshot(
		...
		log "Informe: ${appUrbanCode}:${componenteUrbanCode}:${snapshot}"
		log "folder: " + folder
		...
		if (grupoUrbanCode != null && grupoUrbanCode != "\$grupoUrbanCode") {
			...
			artefactos = findArtifacts(componenteUrbanCode, versionTxt)
			if (artefactos != null) {
				...
				List<MavenCoordinates> coordList = context.versionResolver(apiNexus, snapshotsRepository, folder, componenteUrbanCode, coord);
				log ("coordList -> ${coordList}");
				log ("coordList.size() -> ${coordList.size()}");
				if (coordList != null && coordList.size() > 0) {
					coordList.each {c ->
						...
						//descriptorInd = this.addDocumentationEntries(descriptorInd, build, c, stream, componentName);
					}
			...
			if(rootBuild.getFullDisplayName().contains("-COMP-")) { // Si el job raiz es el de componente se sube el descriptor individual.
				log ("El job raíz es el propio job de componente. Se genera ficha de despliegue individual...");
				...
				TmpDir.tmp { tmpDir ->
					...
					def componentTrimName = componentName.replace(" - ", "_").replace(" -", "_").replace("- ", "_").replace(" ", "_");
					...
					try {
						log "Subiendo el artifactId a la ruta ${grupoUrbanCode}:${artifactId}:${configObject.version}_${componentTrimName} desde ${zip.getCanonicalPath()}"
						NexusHelper.uploadTarNexus(nexusUser, nexusPass, gradleBin, cScriptsStore, "fichas_despliegue", grupoUrbanCode, artifactId, "${configObject.version}_${componentTrimName}", urlNexus, "true", zip.getCanonicalPath(), "zip", {println it});
				...
				log ("El job raíz viene de más arriba que el propio job de componente. No se genera ficha de despliegue individual.");
			}
	 ...
	 * @return descriptor
	 */
	private String addDocumentationEntries(String descriptor, AbstractBuild build, MavenCoordinates coord, String stream, String componentName) {
		def jobName = "${stream} -COMP- ${componentName}"
		...
		if(job != null) {
			job.getProperties().values().each {
				if(it instanceof hudson.model.ParametersDefinitionProperty) {
					...
					// Si existe el parámetro "componenteUrbanCode" y el parámetro
					// "documentacion" los añadimos al descriptor individual.
		...
		if(documentValue != null && documentValue.toString().trim().equals("true")) {
			...
			println("El descriptor individual queda:")
